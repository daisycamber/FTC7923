...
int power1 = 65;// Needs to be a bit more
int power2 = 100;
...
// For the second autonomous program
// false=left, true=right
void turn902(bool direction)
{
	// offsets (negative for less turn, positive for more turn)
	int leftoffset = -13;
	int rightoffset = -5;

	int slowmargin = 40; // the point (before the complete stop) where the robot slows down (for a more accurate turn)
	...
	if(direction)
	{
		// Fast portion of turn
		motor[left] = -1 * fastspeed;
		motor[right] = fastspeed;
		wait1Msec(5);
		// Second one is to prevent what happens if there is an improper heading reading
		while(currHeading < 90 + rightoffset - slowmargin || currHeading > 355)
		{
			wait1Msec(1);
		}
		// Slow portion of turn
		motor[left] = -1 * slowspeed;
		motor[right] = slowspeed;
		while(currHeading < 90 + rightoffset)
		{
			wait1Msec(1);
		}
		stopRobot();
	...
	{
		// Fast portion of turn
		motor[left] = fastspeed;
		motor[right] = -1 * fastspeed;
		wait1Msec(5);
		// Second one is to prevent what happens if there is an improper heading reading
		while(currHeading > 270 - leftoffset + slowmargin || currHeading < 10)
		{
			wait1Msec(1);
		}
		// Slow portion of turn
		motor[left] = slowspeed;
		motor[right] = -1 * slowspeed;
		while(currHeading > 270 - leftoffset)
		{
			wait1Msec(1);
		}
		stopRobot();
...
// For the first autoonomous program
// false=left, true=right
...
void forward(long length)
{
	...
	while(abs(nMotorEncoder(right)) < length || abs(nMotorEncoder(left)) < length)
	{
		motor[left] = (-1 * speed) - (nOffset);
		motor[right] = (-1 * speed) + (nOffset);
	}

	stopRobot();
}

void forward(long length, int nOffset)
{
	nMotorEncoder(right) = 0;
	nMotorEncoder(left) = 0;
	...
	while(abs(nMotorEncoder(right)) < length || abs(nMotorEncoder(left)) < length)
	{
		offset = (nMotorEncoder(right) - nMotorEncoder(left))/8;
		motor[left] = (-1 * speed) - (nOffset) + offset;
		motor[right] = (-1 * speed) + (nOffset) - offset;
	}

	stopRobot();
}
void backward(long length)
{
	nMotorEncoder(right) = 0;
	nMotorEncoder(left) = 0;
	int nOffset = 0;
	...
	{
		offset = (nMotorEncoder(right) - nMotorEncoder(left))/8;
		motor[left] = (speed) - (nOffset);
		motor[right] = (speed) + (nOffset);
	}

	stopRobot();
}

void backward(long length, int nOffset)
{
	nMotorEncoder(right) = 0;
	nMotorEncoder(left) = 0;
...
void autonomous2()
{
	...
	if(compatability) wait1Msec(6000);
...
task main()
{
	...
	nxtDisplayBigStringAt(20,20, "%d", abs(nMotorEncoder(right)));
	wait1Msec(10);
	eraseDisplay();
	}
	}*/
